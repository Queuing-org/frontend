...
describe("viewportDensity", () => {
  it.each([
    ...
    expect(isLaptopCompactViewport({ height, width })).toBe(true);
  ...
  it.each([
    ...
    expect(isLaptopCompactViewport({ height, width })).toBe(false);
    expect(getDesktopViewportDensity({ height, width })).toBe("normal");
  });

  it("가로와 세로 기준을 모두 충족할 때만 compact로 분류한다", () => {
    expect(isLaptopCompactViewport({ height: 901, width: 1600 })).toBe(false);
    expect(isLaptopCompactViewport({ height: 900, width: 1601 })).toBe(false);
  });
});
