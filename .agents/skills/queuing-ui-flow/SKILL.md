...
- Home, search, random, password-room, and direct-URL entry must complete the join handshake before treating the target route as joined. On `room.already-participating`, one shared join-transition owner keeps the socket session lease and original target credential until confirmation succeeds; return, backdrop, and Escape all navigate to the existing room from the error `data.slug`, while retry failures keep the dialog open.
