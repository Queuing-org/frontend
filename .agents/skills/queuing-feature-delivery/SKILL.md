...
10. Before publishing, run `npm run lint`, `npm run test`, and `npm run build` for code changes. Confirm `gh --version` and `gh auth status`, then push the branch and open a draft PR.
    - When `package.json` changes, update `package-lock.json` and run `npm ci`. This repository standardizes on the `packageManager` field and npm; do not add a second package-manager lockfile.
