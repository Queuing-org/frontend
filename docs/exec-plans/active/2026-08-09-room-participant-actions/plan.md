...
- 로그인 사용자가 액션을 수행할 수 있는 다른 참가자 카드에 hover/focus하면 채팅과 같은 `…` trigger를 노출한다.
- `…` trigger를 누르면 채팅과 같은 타이트한 공통 dropdown으로 관리 액션을 제공한다.
- 모든 로그인 사용자는 다른 회원 참가자에게 팔로우/언팔로우, 신고, 차단을 사용할 수 있다.
- 방장은 다른 참가자에게 내보내기를, 다른 회원에게 방장 위임을 추가로 사용할 수 있다.
...
- 로그인 사용자가 본인이 아닌 회원 참가자의 `…`를 누르면 채팅과 동일한 크기의 관리 dropdown이 열린다.
- 같은 `…`를 다시 누르거나 바깥을 누르거나 Escape를 누르면 목록이 닫힌다.
- 일반 로그인 사용자에게는 다른 회원의 현재 관계에 따른 팔로우/언팔로우, 신고, 차단이 표시된다.
- 방장에게만 내보내기와 방장 위임이 추가로 표시된다.
- 게스트는 회원 slug와 채팅 발신자 식별이 필요한 팔로우, 신고, 차단, 방장 위임이 표시되지 않고 내보내기만 표시된다.
- 방장 위임은 `PATCH /api/v1/rooms/{slug}/owner`에 `{ userSlug }`를 전송한다.
- 위임 성공 후 방장과 권한 판정 원본인 room meta를 갱신한다.
...
- [x] 일반 로그인 사용자에게 다른 회원의 `…` 사회 액션 메뉴 복구
