# 칭호 획득 모달 개편 및 후속 시안 교정

## 목표

- `badge-awarded` SSE의 nullable `description`을 보존하고 획득 조건 문구에 사용한다.
- 640×370 모달, 칭호 라벨, 제목·두 문장·두 액션을 최신 시안대로 구성한다.
...
후속 시안 교정은 기존 PR을 이어가므로 `queuing-pr-review-cycle`, `gh-fix-ci`, `gh-address-comments`, `queuing-ui-flow`, `frontend-architecture-guardrails`, `queuing-qa-reviewer`를 적용했다.
...
- 칭호 이름 라벨, `새로운 칭호 획득`, 정확히 두 문장인 달성 안내를 지정된 크기·색상·타이포로 표시한다.
...
- [x] PR #52의 CI와 unresolved review thread 재확인
- [x] 최신 640px·두 문장·텍스트 확인 버튼 시안 교정
- [x] targeted/full QA와 fresh read-only QA
- [x] 후속 커밋 `aeafa57` 준비 및 PR #52 push
