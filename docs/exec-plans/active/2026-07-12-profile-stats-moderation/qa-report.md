...
- CodeRabbit unresolved inline thread 9개를 분류하고 유효한 지적을 반영했다.
- 사용자 차단 직후 sender slug 기반 채팅 제거, 이후 실시간 메시지 제거, 새로고침 후 차단 안내 센티널 제거를 검증했다.
- 설정/방 프로필 이용 시간 하드코딩 UI를 복구하고 추천/비추천 컨트롤을 패널 우측 하단으로 이동했다.
- `npm run lint`: pass
- `npm run test`: pass (12 files, 36 tests)
- `npm run build`: pass
- 최종 판정: `pass`
