...
- 루트 문서를 새로 불러와 기존 TanStack Query 캐시 대신 최신 방 목록을 조회한다.
...
4. 존재하지 않는 방 종료만 전체 문서 교체 이동으로 보강한다.
5. targeted/full QA 후 기능 커밋으로 `dev`와 Draft PR #53에 전달한다.
...
- SPA 캐시를 이어 쓰지 않고 루트 문서를 새로 로드해 방 목록을 다시 조회한다.
